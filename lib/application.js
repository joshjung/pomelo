...
Application.stop = function(force) {
  ...
  var func = this.get(Constants.KEYWORDS.BEFORE_STOP_HOOK),
    stopFunc = this.lifecycleCbs[Constants.LIFECYCLE.BEFORE_SHUTDOWN];

  if (!!stopFunc) {
    stopFunc.call(null, this, shutdown);
  } else if (!!func) {
    utils.invokeCallback(func, self, shutdown);
  } else {
    shutdown();
  }
...
Application.configure = function (env, serverType, fn) {
  ...
  if (env === Constants.RESERVED.ALL || contains(this.settings.env, env)) {
    if (type === Constants.RESERVED.ALL || contains(this.settings.serverType, serverType)) {
      fn.call(this);
    }
  }
