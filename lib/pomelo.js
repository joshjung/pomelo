 ...
 */
fs.readdirSync(__dirname + '/components').forEach(autoLoad.bind(Pomelo, 'components', 'components', true));
fs.readdirSync(__dirname + '/filters/handler').forEach(autoLoad.bind(Pomelo, 'filters', 'filters/handler', true));
fs.readdirSync(__dirname + '/filters/rpc').forEach(autoLoad.bind(Pomelo, 'rpcFilters', 'filters/rpc', false));

function autoLoad(type, _path, defineOnPomelo, filename) {
  if (!/\.js$/.test(filename)) {
    return;
  }

  var name = path.basename(filename, '.js'),
    _load = load.bind(null, './' + _path + '/', name);
  
  console.log('Pomelo autoloading', name);
  this[type].__defineGetter__(name, _load);
