...
module.exports.optComponents = function(comps, method, cb) {
  ...
  async.forEachSeries(comps, function(comp, done) {
    ...
    if (err)
      logger.error('Failed to call method \'%s\' on component: %s\nError was: %j', method, curComp.name, err);
...
var configLogger = function(app) {
  if (process.env.POMELO_LOGGER !== 'off') {
    var env = app.get(Constants.RESERVED.ENV),
      originPath = app.options.logConfigFile || path.join(app.getBase(), Constants.FILEPATH.LOG),
      presentPath = path.join(app.getBase(), Constants.FILEPATH.CONFIG_DIR, env, path.basename(Constants.FILEPATH.LOG));
    ...
    if(fs.existsSync(originPath)) {
      ...
      logger.error('logger file path configuration error.', app.options);
    }
  }
};
